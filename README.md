...
- **多言語対応** — 英語 / 日本語の切り替え
- **複数リポジトリの比較**
- **Issue 依存関係グラフ** — React Flow のインタラクティブなキャンバスで Issue 間の依存関係を可視化。特定の Issue やラベルを起点に部分グラフを表示可能
- **データエクスポート** — JSON / CSV
...
3 つのエピック（`epic: checkout` / `epic: payments` / `epic: infra`）に分かれた 25 件の Issue、21 件の依存関係、20 件の親子関係が入ります。未着手・対応中・完了の 3 状態、関係を持たない Issue、独立した複数のクラスタが含まれるため、起点フィルタの効果も確認できます。
